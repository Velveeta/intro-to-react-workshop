...
const MemoizationTips = () => (
  <React.Fragment>
    <div className="m-1">
      ...
      <div className="mt-3 text-justify">
        One last item to consider is the case when you have an event handler that you need to pass information to from the record level within a list of output components. If you had a list of form inputs, for example, and wanted them all to use the same 'onChange' handler, you could do something like the following.
      </div>
      <div className="mt-3 text-justify">
        <CodeBlock>{`const MyComponent = <div onClick={(e) => { onClickHandler(e) }}>Hello World!</div>;`}</CodeBlock>
      </div>
      <div className="mt-3 text-justify">
        One last item to consider is the case where you have an event handler that needs to be passed to each component in a list, but you need it to pass an identifier back to your component when it's invoked. If you had a list of form inputs, for example, and wanted them all to use the same 'onChange' handler, you could do something like this.
      </div>
      <div className="mt-3 text-justify">
  ...
  render() {
    return (
      ...
      <div className="mt-3 text-justify">
        Each time that _onChange function is called, on each render cycle, it's creating a closure to trap the field name for use later, but then it's returning a new function reference for the handler. When you have a large list of items that you're doing something similar with, this is creating huge numbers of unique function references. Consider moving that handler down a level into the target component, so that it can maintain a single reference on its own instance. Then it can just read the needed props from its own props set, and pass the relevant one(s) back to the callback.
      </div>
      <div className="mt-3 text-justify">
