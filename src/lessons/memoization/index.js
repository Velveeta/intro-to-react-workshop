...
const MemoizationLesson = () => {
  ...
  return (
    <Lesson tips={<Tips />}>
      ...
      <div className="mt-4 text-justify">
        Because of the way React determines what changes to flush to the DOM, at a certain point of scale, applications can start getting sluggish if their maintainers aren't mindful about certain updates. For class-based <Highlight>React.Component</Highlight> instances, if the render cascade makes it down to the component, its output state will be recalculated, and that will be diffed against an in-memory virtual DOM to determine what changes, if any, need to be flushed out to the <i>actual</i> DOM during the reconciliation process. The same goes for standard function-based components as well. So what happens when those components are home to massive trees of other nested components?
      </div>
      <div className="mt-4 text-justify">
        You can start experiencing a delay in feedback from the inception of a dispatched action to the result of that action, when processing incoming user interactions, or dropped frames that make animations and redraw operations janky. In some of the worst-case scenarios, you can cause runaway render cascades, wherein some top-level prop update causes the whole component tree to reevaluate itself, which may cause a value to recompute itself, which may cause it to invoke some callback that propagates the updated value back to the top of the tree, which can cause the whole thing to happen again and again.
      </div>
      <div className="mt-4 text-justify">
        The render lifecycle fires more often than you can possibly imagine (if you don't believe me, try adding a console.log statement to a random component in your large-scale application), and in most cases, it's not a problem. However, in some situations, especially when dealing with large collections of items in the UI, the impact to the overall experience of using the page can be dramatically impacted. One technique to help clue React in as to what has actually been updated in a collection of components is to make good use of the <Highlight>key</Highlight> property. The <Highlight>key</Highlight> should be a unique identifier, which React can use to identify a given record within a list of records in a component, even if those records change their order within the list. It can then more easily determine what might have actually changes in that record's output to the DOM. However, that's not always enough, and this is where intelligent memoization can help tremendously.
      </div>
      <div className="mt-4 text-justify">
        React offers two methods for memoizing components themselves. The React.PureComponent superclass can be used for creating class-based components that you want to have operate in a memoized fashion, and React.memo can be used to create functional components that operate in the same way. When one of these methods is used to create a component, React will do a shallow-comparison check, using an internally-cached copy of the last set of props and state that were used in the target component, against the current set of props and state being used in the target component. If all of those items pass a strict-equality check, React squashes the render cascade at that level, and nothing is evaluated from that point down in the component tree.
      </div>
      <div className="mt-4 text-justify">
        Because of the additional shallow comparison that React performs against data sets on components, there is a slight bit of overhead associated with creating <Highlight>pure</Highlight> components. The goal is to use them where it makes sense, and where the likely result is that you're going to save time overall by performing this extra check against data sets that are actually going to pass the strict equality check. If you have a huge list of components that's prone to change more often than not, then by declaring them as pure components, you're essentially doubling the work that React is having to do with those nodes. First, it will perform its strict equality checks against all incoming props, and then, when one or more of them has legitimately updated, it will have to calculate the output element state and diff that to the virtual DOM and flush those changes to the actual DOM. Examples where this sort of thing may happen would be if you're calculating some internal object- (e.g. styles object) or array-type (e.g. filtered children) prop, and then passing that down to children. Even though the value may be identical from one render cycle to the next, if the references have changed, it will fail the shallow comparison and recalculate everything.
      </div>
      <div className="mt-4 text-justify">
  ...
  return <Datatable rows={pageData} />;
      ...
      <div className="mt-4 text-justify">
        Even if you've tried to optimize your <Highlight>Datatable</Highlight> (or whatever) component by making it pure, the Paginator component is taking the incoming data and running a slice operation against it, which returns a new array reference every single time this component is rendered. That means your pure <Highlight>Datatable</Highlight> component downstream is always going to fail its strict equality check and recalculate its render. The same thing can happen if you're running filter or map operations on an array and passing that result down as props, or creating new objects as you build up a single record from disconnected data points. Libraries like <ExternalLink href="https://www.npmjs.com/package/reselect">reselect</ExternalLink> and <ExternalLink href="https://www.npmjs.com/package/re-reselect">re-reselect</ExternalLink> can help with situations like these, as well as new React Hooks API functions like <ExternalLink href="https://reactjs.org/docs/hooks-reference.html#usememo">useMemo</ExternalLink>, at the cost of larger memory footprints, depending on how extensively you use them.
      </div>
      <div className="mt-4 text-justify">
      ...
      <NavigationFooter lesson="memoization" />
    </Lesson>
  );
};
