  ...
  return (
    <Lesson tips={<Tips />}>
      ...
      <div className="mt-4 text-justify">
        In React applications, you have options for how to treat form- and form-like components. What are form-like components? They're components that look and behave as normal native form elements would, while not actually being native form elements. Think about a custom Dropdown type of component that offers much more flexibility over styles within the dropdown list, but not actually using a <Highlight>select</Highlight> component to implement selectability. It would still take a list of options, and a value of some kind, but it's a custom, form-like component.
      </div>
      <div className="mt-4 text-justify">
        Uncontrolled form elements are elements that behave as a traditional form element might. It's initialized to a given value, and as the user interacts with it, it tracks its own state, but nothing outside of it is tracking that state. At the time of submission, its value is evaluated and sent to some server endpoint, or aggregated into some data object for transmission to some endpoint, validation function, or anything else you might do with form data.
      </div>
      <div className="mt-4 text-justify">
        Controlled form elements are elements that behave slightly differently than traditional form elements might. In order for interactions to be reflected in the UI, the element needs to subscribe to an onChange event, which will update some upstream state variable and be propagated back down to the component in the form of updated prop values. Those updated prop values are what actually drive the updates in the UI itself.
      </div>
      <div className="mt-4 text-justify">
        You may also opt to treat a generic form-like element as either of those two methods, evaluating whether the consumer has passed a <Highlight>value</Highlight> and <Highlight>onChange</Highlight> prop (and if so, treating it as controlled), vs a <Highlight>defaultValue</Highlight> or no value-type prop whatsoever, and treating it as uncontrolled, keeping track of its own state internally.
      </div>
      <div className="mt-4 text-justify">
      ...
      <div className="mt-4 text-justify">
        In this lesson's exercise, you'll be implementing controlled, uncontrolled, and partially-controlled variants of a custom form-like component. It doesn't offer any native form elements internally, so you'll need to implement your own interactions in order to determine when the value has changed, and provide mechanisms for propagating that change to where it needs to go, either to track its own internal state in the case of an uncontrolled component, or alert its parent component to a change in its value via an <Highlight>onChange</Highlight> event listener. Open the project's <FilePath>src/lessons/controlling-form-components/exercise.js</FilePath> file to get started working with different ways of controlling form-like elements in React.
      </div>
      <div className="mt-4 text-justify">
        <ExerciseSandbox>
          <Exercise />
      ...
      <NavigationFooter lesson="controlling-form-components" />
    </Lesson>
  );
};
