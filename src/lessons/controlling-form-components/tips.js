  ...
  <React.Fragment>
    <div className="m-1">
      <div className="text-justify">
        React expects its form-like components to provide either a <Highlight>value</Highlight> or a <Highlight>defaultValue</Highlight> prop. If you supply a <Highlight>defaultValue</Highlight> prop, it will be used to simply initialize the value, but that value won't be tracked over the life of the component. Supplying a <Highlight>value</Highlight> prop, on the other hand, will initialize the value, and will be used from that point onward to set the value of that element in your UI.
      </div>
      <div className="mt-3 text-justify">
        This means that if you pass a <Highlight>value</Highlight> prop to your component, you also need to provide a method by which that value can be updated, in order to be reflected in the UI. This is typically by way of an <Highlight>onChange</Highlight> event handler, but doesn't necessarily need to be an 'onChange' event handler.
      </div>
      <div className="mt-3 text-justify">
