...
const TestingReactComponentsLesson = () => {
  ...
  return (
    <Lesson tips={<Tips />}>
      ...
      <div className="mt-4 text-justify">
        <div>For the purposes of this lesson, I'll define the vocabulary used, so there's no confusion as to what's meant when certain phrases are used:</div>
        <div>
          <ul>
            <li>Unit tests are very granular tests that test inputs to outputs. Functions should ideally be pure and <ExternalLink href="https://simple.wikipedia.org/wiki/Idempotence">idempotent</ExternalLink>. Given the same set of inputs, you should always have the same output, so that you can automate the aspect of calling it with predetermined inputs, and assert that the outputs match the expected values.</li>
...
describe('MyComponent', () => {
  describe('Reducer', () => {
    describe(MY_ACTION_TYPE, () => {
      ...
      <div className="mt-4 text-justify">
        Jest is often paired with another library called Enzyme for specific aspects of testing React components. Enzyme allows you to virtually mount components in isolated conditions, in order to test them individually, apart from the views they may be made a member of. You can then validate, in a sandboxed way, whether the input props of your component yield the expected side-effects, whether those are registered event listeners, CSS class names in the generated markup, specific child components being present, or anything else. You can also programmatically change props and state conditions to ensure that related changes are also present in the component. Once you can validate all of those conditions in an isolated format, it doesn't matter what other component it's made a part of, you can be ensured that if it's passed the correct props, this component will behave in the correct way. This is true component-level testing: validating that your component wholly encapsulates its behaviors and UI, and that those are a reflection of whatever state, props, or context made available to it, no matter where it's nested within your application. You can even choose what level of mounting you want to do with the tested component, so that you can assert generically that certain subcomponent types are present (shallow rendering), or assert more specifically whether certain markup structures are present (mount/render), depending on how deeply you want to test them. A sample set of tests for a React component using Jest and Enzyme might look like this:
      </div>
      <div className="mt-4 text-justify">
...
describe('MyComponent', () => {
  describe('render', () => {
    ...
    it('should initialize its state.value to the value it receives from props', () => {
      const component = shallow(<MyComponent {...props} />);

      expect(component.state('value')).toBe(props.value);
    });
  });
});`}</CodeBlock>
      </div>
...
describe('MyComponent', () => {
  describe('render', () => {
    ...
    it(\`should call our onClick function when a SomeListItemComponent component is clicked, and pass its value\`, () => {
      ...
      <NavigationFooter lesson="testing-react-components" />
    </Lesson>
  );
};
