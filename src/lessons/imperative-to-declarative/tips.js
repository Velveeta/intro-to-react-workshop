...
const ImperativeToDeclarativeTips = () => (
  <React.Fragment>
    <div className="m-1">
      ...
      <div className="mt-3 text-justify">
        When creating a component for some kind of API wrapper, don't just think a single layer deep. If you look at the Theremin/Tone classes in this lesson, you'll see that the API connection itself is set up as a purpose-made wrapper for the <Highlight>AudioContext</Highlight> API. The Theremin UI component itself consumes that in order to make a visual interaction space for that API. When authoring your own API wrapper components, try to keep the wrapper as specific to that API's interface as possible, and leave specific ways of consuming it to external components that can offer different ways of consuming it.
      </div>
    </div>
  </React.Fragment>
);
